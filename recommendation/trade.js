...
function determineTradeDirection(technicalAnalysis, sentimentAnalysis, volumeAnalysis) {
  ...
  if (totalBullish > totalBearish && totalBullish - totalBearish >= 2) {
    return "long"
  } else if (totalBearish > totalBullish && totalBearish - totalBullish >= 2) {
    return "short"
  } else if (totalBullish > totalBearish) {
    return "long"
...
function calculateConfidence(technicalAnalysis, sentimentAnalysis, volumeAnalysis, direction) {
  ...
  if (
    technicalAnalysis.trendStrength.strength === "strong" &&
    technicalAnalysis.trendStrength.direction === (direction === "long" ? "bullish" : "bearish")
  ) {
    confidenceScore += 2
  } else if (
    technicalAnalysis.trendStrength.strength === "moderate" &&
    technicalAnalysis.trendStrength.direction === (direction === "long" ? "bullish" : "bearish")
  ) {
    confidenceScore += 1
  }
  ...
  if (
    technicalAnalysis.momentum.overallMomentum.includes("strongly") &&
  ...
  if (
    technicalAnalysis.movingAverages.maTrend.includes("strongly") &&
  ...
  if (
    sentimentAnalysis.overallSentiment.classification.includes("very") &&
  ...
  if (
    volumeAnalysis.volumeAtPrice.volumeWallClassification ===
...
function generateReasoning(technicalAnalysis, sentimentAnalysis, volumeAnalysis, direction) {
  ...
  if (direction === "long") {
    ...
    if (technicalAnalysis.movingAverages.maTrend.includes("bullish")) {
      reasons.push(
        `Price is trading above key moving averages (${technicalAnalysis.movingAverages.aboveSma20 ? "above SMA20" : ""} ${technicalAnalysis.movingAverages.aboveSma50 ? "above SMA50" : ""} ${technicalAnalysis.movingAverages.aboveSma200 ? "above SMA200" : ""})`,
      )
    }
    ...
    if (technicalAnalysis.movingAverages.maTrend.includes("bearish")) {
      reasons.push(
        `Price is trading below key moving averages (${!technicalAnalysis.movingAverages.aboveSma20 ? "below SMA20" : ""} ${!technicalAnalysis.movingAverages.aboveSma50 ? "below SMA50" : ""} ${!technicalAnalysis.movingAverages.aboveSma200 ? "below SMA200" : ""})`,
      )
    }
  ...
  if (sentimentAnalysis.social.sentiment.includes(direction === "long" ? "bullish" : "bearish")) {
    reasons.push(
      `Social media sentiment is ${sentimentAnalysis.social.sentiment} with ${sentimentAnalysis.social.volume} volume`,
    )
  }
  ...
  if (volumeAnalysis.buySellRatio.pressureClassification === (direction === "long" ? "buying" : "selling")) {
    reasons.push(
      `${direction === "long" ? "Buying" : "Selling"} pressure is dominant with a ${volumeAnalysis.buySellRatio.ratio.toFixed(2)} buy/sell ratio`,
    )
  }
  ...
  if (direction === "long") {
    reasons.push(
      `Key risk: Stop loss placed at ${technicalAnalysis.supportResistance.nearestSupport ? "support level" : "5% below entry"}`,
    )
  } else {
    reasons.push(
      `Key risk: Stop loss placed at ${technicalAnalysis.supportResistance.nearestResistance ? "resistance level" : "5% above entry"}`,
    )
  }
